...
func _unhandled_input(event: InputEvent) -> void:
	...
	elif event is InputEventScreenDrag and event.index == _right_touch_index:
		var delta = event.position - _right_touch_last_pos
		_right_touch_last_pos = event.position
...
func _physics_process(delta):
	...
	if _body:
		...
		if Input.is_action_pressed("shoot") and not _is_reloading and not OS.has_feature("mobile"):
